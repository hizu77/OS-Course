...
report_file="report_mem2_bash.log"
...
log() {
	...
	p_info="$( top -b -n 1 -p "$mem1_pid", "$mem2_pid" | tail -n 2)"
...
while kill -0 "$mem1_pid" 2>/dev/null || kill -0 "$mem2_pid" 2>/dev/null; do
	log
	sleep 1s
done
